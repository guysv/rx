...
/// Priority tier of a key binding. Higher tier wins when multiple
/// bindings match the same input.
#[derive(PartialEq, Eq, Clone, Debug, Default)]
pub enum BindingTier {
    /// From `:map` — applies to Normal + Visual.
    #[default]
    General,
    /// From `:map/normal`, `:map/visual`, `:map/help` — applies to a
    /// fixed set of builtin modes.
    ModeSpecific(Vec<Mode>),
    /// From `:map/script "name"` (or `rx.bind`) — applies to script
    /// modes whose name starts with the binding's name.
    Script(ModeString),
}

impl BindingTier {
    /// Numeric priority for tier comparison. Higher value wins.
    fn priority(&self) -> u8 {
        match self {
            Self::General => 0,
    ...
    pub fn mode_matches(&self, mode: Mode) -> bool {
        match &self.tier {
            ...
            BindingTier::Script(name) => match mode {
                Mode::Script(ref current) => current.as_str().starts_with(name.as_str()),
                _ => false,
            },
        }
    }

    /// For script-tier bindings, the length of the mode-name prefix
...
impl KeyBindings {
    /// Add a key binding.
    ///
    /// Only replaces an existing binding with the same
    /// `(input, state, modifiers, tier)`. The tier embeds the mode
    /// information, so equality on it decides whether two bindings
    /// occupy the same slot; bindings at different tiers coexist and
    /// lookup picks the winner by priority.
...
impl Session {
    ...
    fn handle_keyboard_input(
        ...
        if let Some(key) = key {
            ...
            // except script-tier bindings, which plugins rely on during
            // drag interactions (e.g. axis toggles while rotating).
            if self.mouse_state == InputState::Pressed {
                let binding = self
                    .key_bindings
                    .find(Input::Key(key), modifiers, state, self.mode);
                if !matches!(
                    binding.as_ref().map(|kb| &kb.tier),
                    Some(BindingTier::Script(_))
                ) {
                    return;
                }
            }
...
mod test {
    ...
    fn test_tier_general_vs_mode_specific() {
        // General binding (Normal + Visual); mode-specific for Visual
        // only. In Normal, general wins; in Visual, mode-specific wins.
        let mut kbs = KeyBindings::default();
    ...
    fn test_tier_specialized_does_not_remove_general() {
        // Adding a mode-specific binding must NOT remove a general one.
        let mut kbs = KeyBindings::default();
        let modifiers = ModifiersState::default();
        let state = InputState::Pressed;
    ...
    fn test_script_prefix_match() {
        // Script bindings match by mode-name prefix; the longest
        ...
        // "foobaz" matches the "foo" prefix only.
        assert_eq!(
            kbs.find(
                Input::Key(platform::Key::A),
                modifiers,
                state,
                Mode::Script(ModeString::try_from_str("foobaz").unwrap()),
            ),
            Some(foo_binding),
        );

        // "foobar" matches both; the longer prefix wins.
        assert_eq!(
            kbs.find(
                Input::Key(platform::Key::A),
                modifiers,
                state,
                Mode::Script(ModeString::try_from_str("foobar").unwrap()),
            ),
            Some(foobar_binding),
        );
    }
    ...
    fn test_script_tier_beats_general() {
        ...
        // The script mode: script tier wins.
        assert_eq!(
            kbs.find(
                Input::Key(platform::Key::A),
                modifiers,
                state,
                Mode::Script(ModeString::try_from_str("mymode").unwrap()),
            ),
            Some(script),
        );
    }
